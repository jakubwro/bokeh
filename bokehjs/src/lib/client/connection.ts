...
export class ClientConnection {
  ...
  protected _pending_replies: { [key: string]: [MessageResolver, Rejecter] } = {}
  ...
  protected _repull_session_doc(resolve: SessionResolver, reject: Rejecter): void {
    ...
    this._pull_doc_json().then(
      ...
      // Using a chained call to `catch` to make sure
      // that errors within `then` are also handled.
    ).catch((error) => {
      console.trace?.(error)
  ...
  protected _on_close(event: CloseEvent, reject: Rejecter): void {
    ...
    Object.values(this._pending_replies).forEach(([, reject]) => reject("Disconnected"))
    this._pending_replies = {}
  ...
  protected _steady_state_handler(message: Message): void {
    ...
    if (reqid in this._pending_replies) {
      const resolve = this._pending_replies[reqid][0]
